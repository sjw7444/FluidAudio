...
struct TextNormalizer {
    ...
    /// Replacement tables are applied longest key first (lexical tie-break):
    /// Swift dictionaries iterate in a per-instance random order, and
    /// overlapping keys ("n't" vs "'t", "'d been" vs "'d") make the result
    /// order-dependent. Sorting once here keeps normalization deterministic
    /// and lets the most specific rule win.
    private static let sortedBritishToAmerican = britishToAmerican.sorted(by: longestKeyFirst)
    private static let sortedAbbreviations = abbreviations.sorted(by: longestKeyFirst)
    private static let sortedContractions = contractions.sorted(by: longestKeyFirst)
    private static let sortedNumberWords = numberWords.sorted(by: longestKeyFirst)
    ...
    static func normalize(_ text: String) -> String {
        ...
        for (british, american) in sortedBritishToAmerican {
        ...
        // Abbreviations (applied before bracket/punctuation stripping)
        ...
        for (word, digit) in sortedNumberWords {
